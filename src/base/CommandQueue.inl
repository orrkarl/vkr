    ...
    Buffer& buffer, const T& value, size_t count, std::vector<EventView>& waits, size_t offset)
{
    ...
    auto status
        = clEnqueueFillBuffer(m_object, buffer.view().rawHandle(), &value, sizeof(T), sizeof(value) * offset,
            sizeof(value) * count, static_cast<U32>(rawWaits.size()), rawWaits.data(), &ret);
    ...
    std::vector<EventView>& waits, const NDRange<Dim>& offset)
{
    ...
    auto status = clEnqueueNDRangeKernel(m_object, kernel.rawHandle(), Dim, range.global.data,
        range.local.data, offset.data, static_cast<U32>(rawWaits.size()), rawWaits.data(), &ret);
    if (status != CL_SUCCESS) {
