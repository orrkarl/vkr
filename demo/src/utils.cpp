    ...
    const nr_uint binRasterWorkGroupCount, nr::FrameBuffer frameBuffer)
{
	...
	const nr_uint floatsPerVertex = dim + 1;
	const nr_uint verteciesPerTriangle = 3;
	const nr_uint verteciesPerSimplex = dim;
	const nr_uint floatsPerTriangle = floatsPerVertex * verteciesPerTriangle;
	const nr_uint trianglesPerSimplex = dim * (dim - 1) * (dim - 2) / 6;
	const nr_uint triangleCount = trianglesPerSimplex * simplexCount;
	const nr_uint totalFloatCount = floatsPerTriangle * triangleCount;
	const nr_uint floatsPerSimplex = floatsPerVertex * verteciesPerSimplex;
    
	// Simplex Reducer
	simplexReducer.simplexes = nr::Buffer<nr_float>(CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR, floatsPerSimplex * simplexCount, vertecies, &ret);
    ...
    // Vertex Shader
    vertexShader.points = simplexReducer.result;
	if (nr::error::isFailure(ret)) return ret;
